...
| ErrorCode                      | AppError クラス             | 意味                                                | 返す UC                                                                  |
| ------------------------------ | --------------------------- | --------------------------------------------------- | ------------------------------------------------------------------------ |
| `LLM_GATEWAY_ERROR`            | `LlmGatewayError`           | LLM 呼び出し失敗・空応答・Stage 1/2 JSON パース失敗 | `SendChatMessage` / `RunTutoringPipeline`                                |
...
| 上記以外                                                            | `run_tutoring_pipeline.execute(...)`（Student → Pedagogical → Interface の 3 段） | 成功 → 続行 / 失敗 → **`err(LlmGatewayError)`** |

**Stage 1 / 2 の JSON パース失敗**: Gateway 実装が `LlmGatewayError`（`LLM_GATEWAY_ERROR`）に変換する。リトライは Infrastructure の責務。
