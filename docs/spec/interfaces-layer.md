...
## Tutoring ACL: ITS 3 段プロンプト Builder

`SendChatMessage` は `RunTutoringPipelineUseCase` 経由で 3 段 LLM を呼ぶ。各 Stage のプロンプト組み立ては `interfaces/tutoring/` に置く（[application-usecase.md](./application-usecase.md) の Port 定義を実装）。
...
## Tutoring ACL: ChatPromptBuilder（後方互換）

`ChatPromptBuilder` Port の具象は `interfaces/tutoring/chat_prompt_builder.py` に置く。近い実験の本番経路は ITS 3 段 Builder だが、既存テスト・移行期のため `DefaultChatPromptBuilder` を維持する。
